        ...
        if (isObject(propsOrChildren) && !is2ndArgArrayChildren) {
            ...
            return convertLegacySlots(
                convertLegacyDirectives(
                    createVNode(
                        type,
                        convertLegacyProps(propsOrChildren, type)
                    ),
                    propsOrChildren
                )
            );
...
function convertLegacyProps(
    ...
    if (legacyProps.staticClass) {
        converted.class = normalizeClass([
            legacyProps.staticClass,
            converted.class
        ]);
    }
    if (legacyProps.staticStyle) {
        converted.style = normalizeStyle([
            legacyProps.staticStyle,
            converted.style
        ]);
    }
...
function convertLegacySlots(vnode: VNode): VNode {
    ...
    if (vnode.shapeFlag & ShapeFlags.COMPONENT && isArray(children)) {
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            const slotName =
                (isVNode(child) && child.props && child.props.slot) ||
                'default';
